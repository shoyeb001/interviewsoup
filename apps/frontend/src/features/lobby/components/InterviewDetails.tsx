...
import { useGetInterviewDetailsByRoomIdQuery } from "@/services/interviewApi";
import { useParams } from "react-router";

export default function InterviewDetails() {
    const { roomId } = useParams();
    console.log(roomId)
    const { data, isLoading, isSuccess } = useGetInterviewDetailsByRoomIdQuery(roomId);
    console.log(data);
                ...
                isSuccess && (
                    <>
                        ...
                        <h2 className="text-4xl lg:text-5xl font-bold text-stone-900 tracking-tight leading-tight">
                            Ready for your <br />
                            interview, <span className="text-[#A8441A]">{data.data.interview.candidate_name}?</span>
                        </h2>
                        ...
                        <Card className="border-[#EADFC9] bg-white rounded-2xl p-5 shadow-sm flex flex-col gap-5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xl font-bold text-stone-800">{data.data.interview.company_name}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                <div className="flex items-center gap-1.5 bg-[#FDF0EB] text-[#A8441A] text-xs font-semibold px-3 py-1.5 rounded-md">
                                    ...
                                    Today, {data.data.interview.interview_time}
                                </div>
                                <div className="flex items-center gap-1.5 bg-[#FDF0EB] text-[#A8441A] text-xs font-semibold px-3 py-1.5 rounded-md">
